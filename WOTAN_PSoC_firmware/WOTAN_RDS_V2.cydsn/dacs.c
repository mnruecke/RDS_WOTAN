...
void init_dacs(void){
    ...
    init_dma_dac_3();
    init_dma_dac_4();
    isr_dacs_done_StartEx( isr_DACs_done );
...
void create_wave(void){
    ...
    for(int i=0; i < pts; ++i){
        sig_1.temp_dac_data[i] = (uint8)(
                        (float) 127.5
                        * sin(2.0*M_PI/100 * (float)i*i/pts) + 127.5
                        );  
    }      
    sig_1.temp_dac_data[ DMA_DAC_1_WAVELET_LENGTH-1 ] = DAC_IDLE_VALUE;

    FLASH_Write( sig_1.temp_dac_data, FLASH_DAC_2, DMA_DAC_1_WAVELET_LENGTH); 
...
void run_wave(void){
    ...
    ClockADDA_Stop();
        ...
        cResetClockADDA_Write(1);
        cResetClockADDA_Write(0);
